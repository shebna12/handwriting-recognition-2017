...
import pickle
sys.path.append('/home/lincy/caffe')
...
def load_obj(name ):
    obj_path = "/home/lincy/workflow_structure/Run/SP/short_dict.pkl"
...
def use_svm():
	...
	model_path = home + "workflow_structure/USERS/"+ index.username +"/models/"

	loaded_model = joblib.load(model_path + index.username + "_svm.pkl")
...
def use_dt():
	...
	loaded_model = joblib.load(model_path + index.username + "_rfc.pkl")
	return loaded_model
...
def ensemble_method(letter,set_of_words):
	...
	# cv2.imwrite('/home/shebna/caffe/data/final_sp_temp/dummy/letter.jpg',letter)
	# import sys
	# sys.exit(0)
	# CAFFE_ROOT = '/home/shebna/caffe/'
	
	caffe_folder = home + 'workflow_structure/caffe/'
	examples = index.userfolder + '/examples'
	userfolder =  home+'workflow_structure/USERS/'+ index.username

	BINARY_PROTO_PATH = index.userfolder + '/image_mean.binaryproto'
	DEPLOY_PROTOTXT = index.userfolder + '/lenet_deploy.prototxt'
	CAFFE_MODEL = index.userfolder + '/models/solver_iter_10000.caffemodel'

	# gets image inside of testing/images folder
	# test_image = glob.glob(index.userfolder + "testing/images/*.png")

	# print("test image: ",test_image)

	# IMG_PATH = index.userfolder + '/testing/images/' + test_image



	IMG_PATH = index.userfolder + '/testing/images/' + index.filename
	
	CLASSIFICATION_FILE_PATH = index.userfolder + "logs/"
	...
	if(max(confidences) > 0.60):
		...
		letters,scores = check_duplication(list(letters),list(images))
	...
	print("Set of words: ", set_of_words)
