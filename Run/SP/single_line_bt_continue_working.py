...
def fix_case(processed_word,up_indices):
	final_form = []
	for i,character in enumerate(processed_word):
		if(i in up_indices):
			final_form.append(character.upper())
		else:
			final_form.append(character)

	return final_form


def find_diff_case(word_list):
	up_indices = []
	for word_string in word_list:
		for i,character in enumerate(word_list):
...
def process_possible_words(set_of_words):
	...
	for prod in products:
		...
		word = ''.join(prod)
		words.append(word)
	up_indices = find_diff_case(words)
	words = [x.lower() for x in words]
	words = check_word_duplication(words)
	print("WORDS: ",words)
	final_word = check(words)
	final_word = fix_case(final_word,up_indices)
	return final_word
