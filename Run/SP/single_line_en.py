...
import sys
import cv2
from utils import remove_coinsides,remove_coinsides_letter,remove_noise,sort_LR,get_max_area_contour,resizeTo20,resizeTo28,transform20x20, get_ave_area, closer_to_prev,closer_to_next
import re
...
def find_proba_of(pred_item,proba_all,set_of_words):
	...
	prob = proba_all
	# print("proba: ",prob)
	print("proba of {} : {} " .format(pred_item,prob))
	possibles = [[prob,pred_item]]
...
def process_possible_words(Ppred,set_of_words):
	groupLetters = []
	...
	final_word = correction(final_word)
	return final_word
...
def use_svm():
	...
	loaded_model = joblib.load("shebna_svm_print.pkl")
	...
	return loaded_model
def use_dt():
	# loaded_model = joblib.load("Shebna_small_rfc_print.pkl")
...
def word_recognition(global_ave_area,image):
	...
	for c in contours3: 
		if(i < len(contours3)):
			...
			predicted_label,pred_conf = ensemble_method(letter)
			predicted_label = ''.join(predicted_label)
			set_of_words = find_proba_of(predicted_label,pred_conf,set_of_words)
			print("set_of_words: ",set_of_words)

			# Compile every single recognition to a list
			Ppred.append(predicted_label)
			
			cv2.imshow("letter",letter)
			cv2.waitKey(1)
			# sys.exit(0)
		i = i + 1                      
	final_word = process_possible_words(Ppred,set_of_words)
