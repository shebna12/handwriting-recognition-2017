...
def check_duplication(letters,scores):
	...
	print("cd_letters: ",letters)
	orig_copy = letters[:]
	for item in letters:
		if(orig_copy.count(item) > 1):
			print("Went here")
			dup_item = orig_copy.index(item)
			common_scores.append(scores[dup_item])
			common_letters.append(orig_copy[dup_item])
			del scores[dup_item]
			del orig_copy[dup_item]

	if(len(common_scores) == 0):
		return scores,original_letters
	else:
		return common_scores,common_letters
...
def ensemble_method(letter,set_of_words):
	...
	if(max(confidences) > 0.60):
		...
		possibles_general =list(set(possibles1).union(possibles2))
		possibles_general = sorted(possibles_general,key=lambda b:b[0],reverse=True)
