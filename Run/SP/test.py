	...
	for root, dirs, files in os.walk(folder_path):
		...
		#####------START OF MACHINE LEARNING AND FEATURE EXTRACTION-------####
			...
			for c in contours2:
				...
				# corners = cv2.goodFeaturesToTrack(crop1, 15, 0.02, 10)
				# corners = np.int0(corners)   
				
				#-----Use histogram of gradients instead, using default parameters
				winSize = (64,64)
				blockSize = (16,16)
				blockStride = (8,8) 
				nbins = 9
...
if __name__ == '__main__':
	...
	#TRAINING
	model_hand = svm.SVC(gamma = 0.001, C = 10)
	# model2 = svm.SVC(gamma = 0.0001, C = 100)
	# print("Labels", label)
	...
	# test_image_paths = glob.glob("*.jpg") #Gets all jpg format files
	# X_test,Y_test = feat_extract("test2")
	...
	scoring = model_hand.score(X_test, Y_test)
