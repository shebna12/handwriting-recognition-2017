...
def get_words(true_labels,image,contours2,intersection):
	...
	#adjust height of the y1 to align with the height of the taller contour
		...
		if(i in intersection): 
			...
			predicted_word = word_recognition(word_img,true_labels[nextWord])
...
def new_line(image,rect,line_begin_idx,ix,contours2,differences,sliding,localCut,globalCut,words,true_labels):
...
def lastline(image,line_begin_idx,rect,contours2,differences,sliding,localCut,globalCut,words,true_labels):
...
def segmentation(image,contours2,differences,sliding,localCut,globalCut,words,true_labels):
	...
	for ix in range(len(rect)):
		...
		if rect[ix][1] > line_bottom: #if y is greater than bottom meaning next line
			...
			line_begin_idx,words = new_line(image,rect,line_begin_idx,ix,contours2,differences,sliding,localCut,globalCut,words,true_labels[label_count])
	...
	words = lastline(image,line_begin_idx,rect,contours2,differences,sliding,localCut,globalCut,words,true_labels[label_count-1])
	# sort the last line
	allWords.append(words)
	print (allWords)
...
if __name__ == '__main__':
	...
	for root, dirs, files in os.walk(folder_path):
		for image_path in files:  
			...
			segmentation(image,contours2,differences,sliding,localCut,globalCut,words,true_labels)
